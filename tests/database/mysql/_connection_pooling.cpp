/********************************************************************
  ...
  filename:  aries_base/tests/database/mysql/connection_pooling.cpp

  purpose:   Resource management and concurrency tests for MySQL database
*********************************************************************/
...
void test_statement_reset_reuse() {
  try {
    ...
    assert(db->Execute("CREATE TABLE test (id INTEGER PRIMARY KEY AUTO_INCREMENT, value TEXT)") != nullptr);
...
void test_batch_operation() {
  try {
    ...
    assert(db->Execute("CREATE TABLE test (id INTEGER PRIMARY KEY AUTO_INCREMENT, value INTEGER)") != nullptr);
