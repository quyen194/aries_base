/********************************************************************
  ...
  filename:  aries_base/encryption/hash/md5/md5.cpp
...
std::string MD5::Final() {
  ...
  for (int i = 0; i < 8; ++i) {
    buffer_[56 + i] = static_cast<std::uint8_t>(bit_len >> (8 * i));
  }

  ProcessBlock(buffer_);

  // Convert state to lowercase hex string (little-endian)
  digest_.reserve(32);
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      char hex[3];
      std::snprintf(hex,
                    sizeof(hex),
                    "%02x",
                    static_cast<unsigned int>((state_[i] >> (8 * j)) & 0xff));
      digest_ += hex;
    }
  }
