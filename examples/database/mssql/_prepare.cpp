/********************************************************************
  ...
  created:   2025/12/20 16:53
  filename:  aries_base/examples/database/mssql/prepare.cpp

  purpose:   Use prepared statements safely with MSSQL database
*********************************************************************/
...
void setup() {
  try {
    ...
    db->Disconnect();
    std::cout << "✓ Example setup successful" << std::endl;
  } catch (const std::exception& e) {
    std::cerr << "✗ Example setup failed: " << e.what() << std::endl;
  }
}
// -----------------------------------------------------------------------------

int main() {
  std::cout << "=== MSSQL Database Library - Prepared Statement Example ==="
            << std::endl
            << std::endl;
