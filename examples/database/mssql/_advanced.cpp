/********************************************************************
  ...
  created:   2025/12/20 16:33
  filename:  aries_base/examples/database/mssql/advanced.cpp

  purpose:   Complex real-world scenarios with MSSQL database
*********************************************************************/
...
int main() {
  std::cout << "=== MSSQL Database Library - Advanced Example ===" << std::endl
            << std::endl;
  ...
  try {
    ...
    auto statsResult = db->Execute(R"(
            ...
            FROM (
                SELECT
                    ...
                    p.price,
                    p.stock,
                    (SELECT COUNT(*) FROM orders o WHERE o.product_id = p.id) AS orders_count,
                    COALESCE((SELECT SUM(o.total) FROM orders o WHERE o.product_id = p.id), 0) AS total_revenue
                FROM products p
            ) t
    ...
    auto summaryResult = db->Execute(R"(
            SELECT
                ...
                COUNT(DISTINCT o.product_id) AS products_with_orders
            FROM products p
            LEFT JOIN orders o
